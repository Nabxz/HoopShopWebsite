...
async function fetchUserDetails() {
  try {
    const response = await fetch('http://127.0.0.1:3000/api/user-details', {
      method: 'GET',
      credentials: 'include'
    });
    if (!response.ok) {
...
async function fetchUserAddresses() {
  try {
    const response = await fetch('http://127.0.0.1:3000/api/user-addresses', {
      method: 'GET',
      credentials: 'include'
    });
    if (!response.ok) {
...
async function handleNameUpdate(event) {
  ...
  try {
    ...
    const response = await fetch('http://127.0.0.1:3000/api/update-user-details', {
      method: 'POST',
      headers: {
...
async function handleDeleteAddress(addressId) {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:3000/api/user-addresses/${addressId}`, {
      method: 'DELETE',
      credentials: 'include'
    });
    if (!response.ok) {
...
async function isUserLoggedIn() {
  try {
    const response = await fetch('http://127.0.0.1:3000/api/check-session', {
      method: 'GET',
      credentials: 'include' // Required for cookies to be sent
    });
...
if (loggedIn) {
  try {
    const response = await fetch('http://127.0.0.1:3000/api/cart/delete-item', {
      method: 'POST',
      credentials: 'include', // Required for cookies to be sent
      headers: {
        'Content-Type': 'application/json'
